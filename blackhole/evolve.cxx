...
void smBHEvolve(PARTICLE *pIn, float fBall, int nSmooth, NN *nnList, SMF *smf) {
    ...
    if (smf->bBHFeedback) {
        bhFeedback(pkd, nnList, nSmooth, nAccreted, p, bh, massSum, smf->dConstGamma, smf->dBHFBEff, smf->dBHFBEcrit);
    }
}
...
void combBHEvolve(void *vpkd, void *dst, const void *src) {
    ...
    if (p1.is_gas()) {
        ...
        if (sph.BHAccretor.iPid == NOT_ACCRETED && p2->iAccPid != NOT_ACCRETED) {
