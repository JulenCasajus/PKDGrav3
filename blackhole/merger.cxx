...
int smReSmoothBHNode(SMX smx, SMF *smf, int iSmoothType) {
    ...
    // Here we store the pointers to the particles
    // whose interaction need to be computed
    std::vector <PARTICLE *> sinks;
    sinks.reserve(64); // At = ost, the size of the bucket
    ...
    for (int i = NRESERVED_NODES; i < pkd->Nodes() - 1; ++i) {
        ...
        if (bBHinBucket) {
            ...
            // There is no eligible particle in this bucket, go to the next
            if (sinks.empty()) {
                continue;
            }
                ...
                for (int ix = iStart[0]; ix <= iEnd[0]; ++ix) {
                    ...
                    for (int iy = iStart[1]; iy <= iEnd[1]; ++iy) {
                        ...
                        for (int iz = iStart[2]; iz <= iEnd[2]; ++iz) {
                            ...
                            buildCandidateMergerList(smx, smf, node, bnd_node, &nCnt, r, ix, iy, iz);
                        }
                    }
                }
            } else {
