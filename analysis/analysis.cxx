...
/*
** This function calculates the distance between a particle and a
** reference point. If a periodic boundary is in effect, the smallest
...
** and sort by distance when finished.
*/
...
void pkdCalcMtot(PKD pkd, double *M, uint64_t *N) {
    *N = pkd->Local();
    *M = std::accumulate(pkd->particles.begin(), pkd->particles.end(), 0.0, [](double a, auto &p) {return a + p.mass();});
}
...
static void CalculateInertia(PKD pkd, int nBins, const double *dRadii, SHAPESBIN *shapesBins) {
    ...
    */
    for (i = iBin = 0; i < n; i++) {
        ...
        while (pl[i].d2 < dRadii[iBin] * dRadii[iBin] && iBin < nBins) {
            ++iBin;
        }
        while (pl[i].d2 > dRadii[iBin] * dRadii[iBin]) {
            --iBin;
        }
    ...
    if (pkd->Self() == 0) {
        ...
        for (i = iBin = 0; iBin < nBins; iBin++) {
            ...
            for (j = 0; j < 3; j++) {
                for (k = 0; k < 3; k++) {
                    if (k <= j) {
                        inertia_cm[j + 1][k + 1] = ((pShape->dInertia[j][k] - pShape->com[j] * pShape->com[k] / pShape->dMassEnclosed) / pShape->dMassEnclosed);
                    } else {
                        inertia_cm[j + 1][k + 1] = ((pShape->dInertia[k][j] - pShape->com[k] * pShape->com[j] / pShape->dMassEnclosed) / pShape->dMassEnclosed);
                    }
                }
...
** Density Profile
*/
void pkdProfile(PKD pkd, uint8_t uRungLo, uint8_t uRungHi, const blitz::TinyVector <double, 3> &dCenter,
                const double *dRadii, int nBins, const blitz::TinyVector <double, 3> &com,
                const blitz::TinyVector <double, 3> &vcm, const blitz::TinyVector <double, 3> &L) {
    ...
    /*
    ** Now we add all of the particles to the appropriate bin. NOTE that both
    ...
    */
    ...
    /* We need angular momentum to calculate tangential velocity sigma, so we reopen the cache */
