...
class LinearSignal : public NoiseGenerator {
    ...
    protected:
        void update(gridinfo::complex_vector_t &pencil, gridinfo::complex_vector_t &noise, int j, int k) override;

    public:
        explicit LinearSignal(CSM csm, double a, double Lbox, unsigned long seed, bool bFixed = false, float fPhase = 0);
        ~LinearSignal() override;
};

LinearSignal::LinearSignal(CSM csm, double a, double Lbox, unsigned long seed, bool bFixed, float fPhase) : NoiseGenerator(seed, bFixed, fPhase) {
    auto size = csm->val.classData.perturbations.size_k;
    this->csm = csm;
    iLbox = 2 * M_PI / Lbox;
    acc = gsl_interp_accel_alloc();
    spline = gsl_spline_alloc(gsl_interp_cspline, size);
    logk = static_cast <double *> (malloc(sizeof(double) * size));
    field = static_cast <double *> (malloc(sizeof(double) * size));

    for (auto i = 0; i < size; i++) {
        auto k = csm->val.classData.perturbations.k[i];
        logk[i] = log(k);
        field[i] = csmDeltaRho_pk(csm, a, k) / csmZeta(csm, k);
...
int pstAddLinearSignal(PST pst, void *vin, int nIn, void *vout, int nOut) {
    ...
    if (pstNotCore(pst)) {
        ...
        pstAddLinearSignal(pst->pstLower, vin, nIn, nullptr, 0);
        mdlGetReply(pst->mdl, rID, nullptr, nullptr);
    } else {
...
void MSR::AddLinearSignal(int iGrid, int iSeed, double Lbox, double a, bool bFixed, float fPhase) {
    ...
    in.Lbox = Lbox;
    in.a = a;
    pstAddLinearSignal(pst, &in, sizeof(in), nullptr, 0);
}
